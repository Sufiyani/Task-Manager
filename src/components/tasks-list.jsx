...
function TaskList() {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-4 sm:p-6 min-h-[calc(100vh-140px)]">
      <h2 className="text-xl sm:text-2xl font-bold mb-4 sm:mb-6 text-gray-800 dark:text-white">Task List</h2>
        ...
        <Table className="min-w-full text-sm">
          <TableHeader>
            <TableRow className="bg-gray-100 dark:bg-slate-700">
              <TableHead className="text-black dark:text-white font-semibold">Task Name</TableHead>
              <TableHead className="text-black dark:text-white font-semibold">Description</TableHead>
              ...
              <TableHead className="text-black dark:text-white font-semibold">Complete</TableHead>
      ...
      <div className="lg:hidden space-y-4">
        {tasks.map((task) => (
          <div
            key={task.id}
            className="bg-gray-50 dark:bg-slate-800 rounded-xl p-4 border border-gray-200 dark:border-slate-700 hover:shadow-md transition-shadow"
          >
            {/* Task Header */}
            ...
            {/* Action Buttons */}
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-gray-200 dark:border-slate-600">
              {task.status !== "Completed" ? (
                <button
                  onClick={() => markCompleted(task)}
                  className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-green-700 dark:text-green-400 bg-green-50 dark:bg-green-900/20 rounded-lg hover:bg-green-100 dark:hover:bg-green-900/30 transition-colors"
                >
                  <Check size={16} />
                  Complete
                </button>
              ) : (
                <div className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-green-800 dark:text-green-400 bg-green-100 dark:bg-green-900/20 rounded-lg opacity-75">
                  <Check size={16} />
                  Completed
                </div>
              )}
              <button
                onClick={() => handleEditClick(task)}
                className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-blue-700 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/20 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-900/30 transition-colors"
              >
                <Edit size={16} />
                Edit
              </button>
              <button
                onClick={() => deleteTask(task.id)}
                className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-red-700 dark:text-red-400 bg-red-50 dark:bg-red-900/20 rounded-lg hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors"
              >
                <Trash size={16} />
                Delete
              </button>
